...
class SponsorAcknowledgeCandidate extends Mailable
{
//    use Queueable, SerializesModels;
    ...
    {
        if (!$candidate) {
            throw new NotFoundResourceException('There was no candidate specified. Unable to send message.');
        }
        ...
        $this->subject_line = '[' . config('site.community_acronym') . '] ' . $title . ': ' . $candidate->names;
    ...
    public function build()
    {
        $message = $this->subject($this->subject_line)
            ...
            ->markdown('emails.' . (config('site.preweekend_sponsor_confirmations_enabled') ? 'sponsor-acknowledge-candidate' : 'sponsor-candidate-recorded'));
